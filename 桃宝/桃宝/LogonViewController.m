...
-(UIButton *)landBtn{
    if (!_landBtn) {
       ...
       [_landBtn setImage:[[UIImage imageNamed:@"注册界面下一步按钮"]imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
        [_landBtn addTarget:self action:@selector(methodToNextstepView) forControlEvents:UIControlEventTouchUpInside];
    }
    return _landBtn;
}
-(UIButton *)logonBtn{
    if (!_logonBtn) {
        ...
        [_logonBtn setTitle:@"去登录" forState:UIControlStateNormal];
        [_logonBtn setTitleColor:RGB(25, 152, 189) forState:UIControlStateNormal];
        [_logonBtn addTarget:self action:@selector(methodToLandingView) forControlEvents:UIControlEventTouchUpInside];
...
//        [_btnWeiBo addTarget:self action:@selector(weiBo) forControlEvents:(UIControlEventTouchUpInside)];
    }
    return _btnWeiBo;
}
...
-(void)methodToNextstepView{
    
    NextstepViewController *nexVC = [[NextstepViewController alloc]init];
    [self.navigationController pushViewController:nexVC animated:NO];
}
-(void)QQLogon{
